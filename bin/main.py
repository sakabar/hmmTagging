...
def main():
  ...
  for k in tagger.n_pp.keys():
    if k[0]=="###":
      print tagger.n_pp[k]

  sys.exit(0)
  words = ["Sato", "likes" , "baseball", "."]
  tagged = tagger.viterbi(words)
  print tagged
  sys.exit(0)

  
  #checking
  s, p = utils.get_pair_of_sentence_and_pos(testData[0])
  print tagger.n_wp[("everyone", ",")]
  sys.exit(0)
